...
if __name__ == "__main__":
    ...
    for idx, recruiter in enumerate(recruiters, start=1):
        ...
        job_link = recruiter.get("job_link")

        if job_link:
            email_subject = f"Application for {template_data['role_name']} at {template_data['company_name']} — {template_data['sender_name']}"
            template_data["intro_paragraph"] = (
                f"I just submitted my application for the {template_data['role_name']} role at "
                f"{template_data['company_name']} and wanted to reach out directly - here's the job "
                f"posting for reference: {job_link}"
            )
            template_data["resume_line"] = (
                "I've attached my resume for reference, and would really appreciate it if you could "
                "pass it along to the hiring team."
            )
        else:
            email_subject = f"Exploring {template_data['role_name']} Opportunities at {template_data['company_name']} — {template_data['sender_name']}"
            template_data["intro_paragraph"] = (
                f"I came across your profile and wanted to reach out directly, as I'm currently exploring "
                f"{template_data['role_name']} opportunities and thought {template_data['company_name']} could be a great fit."
            )
            template_data["resume_line"] = (
                f"I've attached my resume for reference. If there's an open {template_data['role_name']} role at "
                f"{template_data['company_name']}, I'd really appreciate the chance to connect and share more about my background."
            )
        ...
        success = send_custom_email(
            ...
            attachment_path="PIYUSH_RESUME.pdf",
